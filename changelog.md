...
- 重点事项 · 思维导图：新增概要、外框（⌘/Ctrl+B）、文字大纲面板
- 思维导图导出 PNG / PDF / Markdown / 纯文本，菜单样式与文档导出统一
- 12 种骨架采用中心对齐连线，修复单子节点错位
- 骨架改为列表式选择，下拉菜单支持外部点击 / Esc 收起
- 快速样式可再次点击取消，清除样式恢复默认
- 全屏编辑保留「更多操作」菜单
- 分栏底部新增左右箭头，可与相邻分栏交换位置
