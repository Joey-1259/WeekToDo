<template>
  ...
  >
    ...
    <div class="mind-map-inspector-content">
      <section class="mind-map-inspector-section">
        <header>
          <strong>骨架</strong>
          <small>控制方向、密度与连接线</small>
        </header>

        <div class="mind-map-skeleton-grid">
          <button
            v-for="skeleton in skeletons"
            :key="skeleton.id"
            type="button"
            class="mind-map-skeleton-card"
            :class="{
              active:
                skeleton.id === activeSkeletonId,
            }"
            @click="
              $emit(
                'change-skeleton',
          ...
          >
            <span
              class="mind-map-skeleton-preview"
        ...
        <label class="mind-map-switch-row">
          <span>
            <strong>紧凑排列</strong>
            <small>减少节点间距，适合大型脑图</small>
          </span>

          <button
            type="button"
            role="switch"
            :aria-checked="String(compact)"
      ...
      <section class="mind-map-inspector-section">
        <header>
          <strong>配色主题</strong>
          <small>控制全图的默认色彩与背景</small>
        </header>

        <div class="mind-map-theme-list">
          <button
